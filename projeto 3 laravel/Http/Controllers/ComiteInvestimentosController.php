...
class ComiteInvestimentosController extends Controller
{
    ...
    {
        $registros = Reuniao::where([['grupo', 'Comite Investimentos'], ['arquivar', 'nao']])->orderBy('data', 'desc')->first();
        if(isset($registros))
            {
                $id = $registros->id;
        ...
        return view("users.comite-investimentos.index", compact("reuniao", "topicos", "documentos"));
    }
    ...
    public function pautasAnteriores()
    {
    	...
    	return view("users.comite-investimentos.pautas-anteriores", compact("registros"));
    }
    ...
    public function visualizarPauta($id)
    {
        ...
        $url_current = str_contains(url()->previous(),'comite-investimentos/pautas-anteriores');

    	//$topicos = conselho::find($id)->topico;
    	$topicos = Topico::where('conselho_id',($id))->orderBy('ordenacao')->get();

    	return view("users.comite-investimentos.visualizar-pauta", compact("reuniao", "topicos", "url_current"))->with('documentos', Documento::all());
    }
    ...
    public function pesquisar(Request $req)
    {
        ...
        return view("users.comite-investimentos.pesquisar", compact("registros"));
